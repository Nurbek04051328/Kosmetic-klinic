...
const create = async (req, res) => {
    try {
        ...
        let newUser =  new User({ login, password:hashPass, role, name, phone, avatar, status, createdAt:Date.now() });
...
const update = async (req, res) => {
    if (req.body._id) {
        let { _id, login, password, type, avatar, name, phone, status } = req.body;
        (type == 1) ? role = 'bugalter' : role = 'inventor'
        if(password) {
            const hashPass = await bcrypt.hash(password, 10);
            password = hashPass;
        ...
        let findUser = await User.findOne({_id:saveUser._id}, '-password').lean();
